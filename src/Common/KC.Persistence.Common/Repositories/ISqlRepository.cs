...
{
    public interface ISqlRepository<TEntity> : IDbRepository<TEntity>
        where TEntity : SqlEntity
    {
        ...
        new ISqlRepository<TEntity> AsNoTracking();
        ...
        /// <returns>The same ISqlRepository implementation so that multiple calls can be chained.</returns>
        new ISqlRepository<TEntity> Include(Expression<Func<TEntity, object?>> navigationPropertyPath);
        ...
        /// <returns>The same ISqlRepository implementation so that multiple calls can be chained.</returns>
        new ISqlRepository<TEntity> Include(string navigationPropertyPath);
